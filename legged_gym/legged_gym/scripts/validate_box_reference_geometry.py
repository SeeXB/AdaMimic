...
def main() -> None:
    ...
    parser.add_argument("--penetration-tolerance", type=float, default=1e-5)
    ...
    left_surface = _surface_distance_to_assigned_side(local[:, 0], 1.0, half_extent, target_clearance, patch_margin)
    right_surface = _surface_distance_to_assigned_side(local[:, 1], -1.0, half_extent, target_clearance, patch_margin)
    ...
    if max_positive_penetration > args.penetration_tolerance:
        raise SystemExit(
            f"Reference hand contact point penetrates the box: max_positive_penetration="
            f"{max_positive_penetration:.6f} > tolerance={args.penetration_tolerance:.6f}"
        )
